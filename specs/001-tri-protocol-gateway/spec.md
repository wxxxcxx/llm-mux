...
- **FR-008**: 系统必须支持可配置的路由规则，基于模型名称、请求特征（是否有工具、是否有媒体）将请求分发到指定后端。
...
- **FR-013**: 系统必须支持 SSE 流式传输，以逐事件方式翻译流式响应，不缓冲完整响应体。
- **FR-014**: 系统必须将后端错误响应（HTTP 错误、协议错误、流中断）映射为客户端协议格式的错误表示。
- **FR-015**: 系统必须透传未知字段（provider extension fields），确保前向兼容性。
- **FR-016**: [NEEDS CLARIFICATION: 是否需要支持请求/响应级别的访问日志？标准包括哪些字段（请求ID、模型、延迟、Token数）？]
- **FR-017**: [NEEDS CLARIFICATION: 是否需要速率限制（rate limiting）和并发连接数控制？阈值如何设定？]
...
- **SC-002**: 请求协议转换延迟 ≤ 1ms（典型请求体 < 10KB），单个流式事件转换延迟 ≤ 100μs。
- **SC-003**: Release 构建的二进制文件大小 < 15MB（Linux x86_64 stripped）。
...
- **SC-007**: LLM Mux 服务在 100 并发请求下，p95 翻译延迟 ≤ 2ms，内存占用 ≤ 50MB。
...
- 暂不支持非标准/私有协议扩展——仅支持 OpenAI 和 Anthropic 已公开文档化的 API 版本。
