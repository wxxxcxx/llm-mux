...
fn parse_format(s: &str) -> Result<genai::adapter::AdapterKind, String> {
    genai::adapter::AdapterKind::from_lower_str(s)
        .ok_or_else(|| format!("unknown adapter format: {s}"))
}
...
fn parse_protocol(s: &str) -> Result<Protocol, String> {
    match s {
        ...
        _ => Err(format!("unknown inbound protocol: {s}")),
    }
}
...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (name, p) in &self.providers {
            parse_format(&p.format).map_err(|e| format!("provider '{name}': {e}"))?;
        }
    ...
    pub fn to_router(&self) -> Result<ConfigurableRouter, String> {
        ...
        for (name, p) in &self.providers {
            let format = parse_format(&p.format)?;
