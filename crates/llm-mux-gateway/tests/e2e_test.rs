...
"#,
        ...
        pub fn validate_chat_error(body: &[u8], expected_code: u16, expected_message: &str) {
            let data: Value = serde_json::from_slice(body)
                .unwrap_or_else(|_| panic!("not valid JSON error response: {}", String::from_utf8_lossy(body)));
            assert!(
    ...
    async fn test_empty_request_body() {
        ...
        assert!(status.is_client_error() || status.is_server_error(), "empty body should error");
    }
    ...
    async fn test_whitespace_only_body() {
        ...
        assert!(status.is_client_error() || status.is_server_error(), "whitespace body should error");
    }
    ...
    async fn test_injection_like_prompt() {
        ...
        assert!(resp.status().as_u16() < 600, "injection prompt should not crash");
    }
    ...
    async fn test_concurrent_different_models() {
        let models = vec!["deepseek-v4-flash", "deepseek-v4-pro", "glm-5.1", "kimi-k2.5", "kimi-k2.6"];
        let (addr, api_key) = start_server().await;
        let api_key = Arc::new(api_key);
        let mut handles = Vec::new();
        for m in models {
    ...
    async fn test_multiturn_tool_calls() {
        ...
        let status = resp.status();
        let text = resp.text().await.unwrap();
        eprintln!("[tool-calls] status={status} body={}", &text[..text.len().min(300)]);
        if !status.is_success() {
            return;
        }
    }
...
fn test_response_format_roundtrip() {
    ...
    let ir = codec.decode_request(body.as_bytes()).unwrap();
    let fmt = ir.response_format.as_ref().expect("response_format should be present");
